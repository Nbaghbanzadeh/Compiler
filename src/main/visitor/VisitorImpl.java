...
public class VisitorImpl implements Visitor {
    ...
    private void checkInitial(ActorInstantiation ai) {
            ...
            if(findArg.split("#", 3)[2].equals(ai.getType().toString())
                ...
                for (j = 0; j < ai.getInitArgs().size(); ++j) {
                    ...
                    if(!argMain.get(i + j).split("#", 3)[2].equals(ai.getType().toString())) {
                        errorMessages.add(ai.getLine() + "#arguments do not match with definition1");
                        return;
                    }
                    ...
                    if(!whatType.equals(ai.getInitArgs().get(j).getType().toString())
                        ...
                        errorMessages.add(ai.getLine() + "#arguments do not match with definition2");
                        return;
                    }
                }
                if(j < argMain.size()) {
                    if (argMain.get(i + j).split("#", 3)[0].equals(ai.getType().toString())) {
                        errorMessages.add(ai.getLine() + "#arguments do not match with definition3");
                        return;
                    }
                }
                if(entered)
        ...
        if(noInput && ai.getInitArgs().size() != 0) {
            errorMessages.add(ai.getLine() + "#arguments do not match with definition4");
            return;
        }
        if(noInput && ai.getInitArgs().size() > 0) {
            errorMessages.add(ai.getLine() + "#arguments do not match with definition5");
            return;
        }
        if(!noInput && ai.getInitArgs().size() == 0) {
            errorMessages.add(ai.getLine() + "#arguments do not match with definition5");
    ...
    private void checkMsgHandlerArg(MsgHandlerCall call, Type type) {
        for (Expression arg : call.getArgs()) {
            arg.accept(this);
        }
        boolean noInput = true;
        for(int i = 0; i < msgHandlerArgsNames.size(); i++) {
            String findArg = msgHandlerArgsNames.get(i);
            if (findArg.split("#", 3)[2].equals(type.toString())
                    && findArg.split("#", 3)[0].equals(call.getMsgHandlerName().getName())) {
                noInput = false;
                boolean entered = false;
                int j;
                for (j = 0; j < call.getArgs().size(); ++j) {
                    entered = true;
                    if (!msgHandlerArgsNames.get(i + j).split("#", 3)[2].equals(type.toString())) {
                        msgHandlerArgsNames.add(call.getLine() + "##arguments does not match with definition in msgHandlerCall");
                        return;
                    }
                    String whatType = msgHandlerArgsNames.get(i + j).split("#", 3)[1];
                    if (!whatType.equals(call.getArgs().get(j).getType().toString())
                            && !call.getArgs().get(j).getType().toString().equals("notype")) {
                        errorMessages.add(call.getLine() + "#arguments does not match with definition in msgHandlerCall");
                        return;
                    }
                }
                if (j < msgHandlerArgsNames.size()) {
                    if (msgHandlerArgsNames.get(i + j).split("#", 3)[0].equals(type.toString())) {
                        errorMessages.add(call.getLine() + "#arguments does not match with definition in msgHandlerCall");
                        return;
                    }
                }
        ...
        if(noInput && call.getArgs().size() != 0) {
            errorMessages.add(call.getLine() + "##arguments does not match with definition in msgHandlerCall");
            return;
        }
        if(noInput && call.getArgs().size() > 0) {
    ...
    public String findSizeRight(Assign assign) {
        String rightSize = "";
    ...
    public String findSizeLeft(Assign assign) {
        String leftSize = "";
    ...
    public void visit(MsgHandlerCall msgHandlerCall) {
        if(msgHandlerCall.getInstance() instanceof Self) {
            ...
            checkMsgHandlerArg(msgHandlerCall, new ActorType(new Identifier(currentActor)));
        }
            ...
            checkMsgHandlerArg(msgHandlerCall, msgHandlerCall.getInstance().getType());
        }
    }
    ...
    public void visit(ActorInstantiation actorInstantiation) {
        ...
        boolean noInput = true;
        for(int i = 0; i < knownActorNamesMain.size(); i++) {
            String findKnownActors = knownActorNamesMain.get(i);
            if(findKnownActors.split("#", 3)[0].equals(actorInstantiation.getType().toString())) {
                noInput = false;
                boolean entered = false;
                int j;
                for (j = 0; j < actorInstantiation.getKnownActors().size(); ++j) {
                    entered = true;
                    if(!knownActorNamesMain.get(i + j).split("#", 3)[0].equals(actorInstantiation.getType().toString())) {
                        errorMessages.add(actorInstantiation.getLine() + "#knownactors does not match with definition1");
                        checkInitial(actorInstantiation);
                        return;
                    }
                    String whatType = knownActorNamesMain.get(i + j).split("#", 3)[2];
                    if(!whatType.equals(actorInstantiation.getKnownActors().get(j).getType().toString())
                            && !actorInstantiation.getKnownActors().get(j).getType().toString().equals("notype")) {
                        errorMessages.add(actorInstantiation.getLine() + "#knownactors do not match with definition2");
                        checkInitial(actorInstantiation);
                        return;
                    }
                }
                if(j < knownActorNamesMain.size()) {
                    if (knownActorNamesMain.get(i + j).split("#", 3)[0].equals(actorInstantiation.getType().toString())) {
                        errorMessages.add(actorInstantiation.getLine() + "#knownactors do not match with definition3");
                        checkInitial(actorInstantiation);
                        return;
                    }
                }
        ...
        if(noInput && actorInstantiation.getKnownActors().size() != 0) {
            errorMessages.add(actorInstantiation.getLine() + "#knownactors do not match with definition4");
            checkInitial(actorInstantiation);
            return;
        }
        if(noInput && actorInstantiation.getKnownActors().size() > 0) {
            errorMessages.add(actorInstantiation.getLine() + "#knownactors do not match with definition5");
            checkInitial(actorInstantiation);
            return;
        }
        if(!noInput && actorInstantiation.getKnownActors().size() == 0) {
